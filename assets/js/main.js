...
function initDropdownMenus() {
  document.querySelectorAll('.nav-item').forEach(item => {
    ...
    if (link && menu) {
      link.addEventListener('click', (e) => {
        e.preventDefault();
        const isOpen = menu.style.visibility === 'visible' || menu.style.opacity === '1';
        isOpen ? closeAllDropdowns() : openDropdown(item);
      });
    }
...
function initRTL() {
  ...
  const syncLabels = (dir) => {
    ...
    document.querySelectorAll('.dir-toggle, .rtl-toggle').forEach(btn => {
      ...
      /* Swap the direction icon */
      const icon = btn.querySelector('.dir-toggle-icon');
      if (icon) {
        icon.classList.remove('bi-text-left', 'bi-text-right', 'bi-arrow-left-right');
